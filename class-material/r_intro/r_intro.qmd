...
subtitle: "(Part 1)<br><span style='font-size: 0.8em; color: #ff5a5a; margin-top: 1em;'>Push the down arrow key ↓ to advance</span>"
...
Note that you can click on line 2 and edit the code and re-run.  
*Give this a try now before moving on...*

## Checking your understanding

We will use these types of "code chunks" to ask you to complete some missing code, typically indicated by the blank [______]{style="color:black;"} "underline" place holder.  
</br>
...
You will make lots of assignments and the wee arrow operator `<-` can be a pain to type.  

However, **don't be lazy and use `=`**, although it would work, because it will just sow confusion later when we learn about functions - *we always use `=` for setting function inputs* (more on this later). 

</br>

> In *RStudio* folks will often utilize the **keyboard shortcut**: `Alt`/`Option` + `-` (that is pushing the `Alt` and `minus` keys together on a PC or the `Option` and `minus` keys if you are on a Mac).


## Side note about object names... {.nb-slide}

Object names can be anything you want with a couple of notable exceptions. They cannot start with a digit and can not contain certain "special" characters such as a comma or a space. 
...
What happened? What about the 2nd line? Did you run both chunks? Can you fix this? *Edit the last code chunk to fix the error here...*
...
Try editing the previous code chunk to read `seq(1, 10)`. This will happely work and demonstrates something about how R resolves function arguments when they are not spelled out with `=` signs. 
...
## Finally, a more advanced example...

Can you figure out what the following code does and discuss it with your neighbor?
...
ind <- which(a!=b)
paste(a[ind], ind, b[ind], sep="")
```
- What are the tree `functions()` in this code chunk and what do they do?
- Can you change this code to print out the identical nucleotides?


# End of Part 1
Well done for getting this far !  
Feel free to check out our [R cheat sheet](https://bioboot.github.io/bimm143_W25/class-material/base-r.pdf) and/or close this now :grinning:
